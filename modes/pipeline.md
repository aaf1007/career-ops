...
Processes accumulated offer URLs in `data/pipeline.md`. The user adds URLs whenever and then runs `/career-ops pipeline` to process them all.
...
3. **If 3+ URLs are pending**, launch agents in parallel (Agent tool with `run_in_background`) for maximum speed.
